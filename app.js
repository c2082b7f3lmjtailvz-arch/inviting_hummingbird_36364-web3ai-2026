...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 出納帳の年間表示・月別詳細のドリルダウン状態
  let ledgerYear = new Date().getFullYear();
  let ledgerDetailYear = null;
  let ledgerDetailMonth = null;
  ...
  const btnGotoCashbook = document.getElementById('btn-goto-cashbook');
  const calendarMonthExpense = document.getElementById('calendar-month-expense');
  ...
  const ledgerYearView = document.getElementById('ledger-year-view');
  const ledgerYearLabel = document.getElementById('ledger-year-label');
  const ledgerYearGrid = document.getElementById('ledger-year-grid');
  const btnLedgerPrevYear = document.getElementById('btn-ledger-prev-year');
  const btnLedgerNextYear = document.getElementById('btn-ledger-next-year');

  const ledgerMonthDetail = document.getElementById('ledger-month-detail');
  const btnLedgerBackToYear = document.getElementById('btn-ledger-back-to-year');
  ...
  btnGotoCashbook.addEventListener('click', () => {
    switchTab('cashbook');
    showLedgerMonthDetail(currentMonth.getFullYear(), currentMonth.getMonth());
  });
  ...
  // 指定日・時間帯と重なる既存予定を返す（開始時刻未設定の予定は対象外）
  function findScheduleConflicts(dateStr, startTime, endTime) {
    if (!startTime) return [];
    const newStart = timeToMinutes(startTime);
    const newEnd = endTime ? timeToMinutes(endTime) : newStart;

    return (schedules[dateStr] || [])
      .filter(evt => {
        if (!evt.startTime) return false;
        const evtStart = timeToMinutes(evt.startTime);
        const evtEnd = evt.endTime ? timeToMinutes(evt.endTime) : evtStart;

        // どちらも終了時刻未設定（一点の時刻）の場合は完全一致のみ重複とみなす
        if (newEnd === newStart && evtEnd === evtStart) {
          return newStart === evtStart;
        }
  ...
  // 重複がある場合、確認ダイアログを出して続行するかどうかを返す（重複なしなら常にtrue）
  function confirmScheduleConflicts(conflicts, title) {
    if (conflicts.length === 0) return true;
    const lines = conflicts.map(({ dateStr, evt }) => {
      const timeLabel = evt.startTime ? `${evt.startTime}${evt.endTime ? '〜' + evt.endTime : ''}` : '';
      return `${formatMonthDay(dateStr)} 「${evt.title}」${timeLabel ? `(${timeLabel})` : ''}`;
    });
    return confirm(`以下の予定と時間が重なっています。\n${lines.join('\n')}\n\n「${title}」を追加しますか？`);
  }
  ...
  function renderCalendar() {
    ...
    // 出納帳タブへのリンク帯（今月の支出）を表示中の月に合わせて更新
    calendarMonthExpense.textContent = formatYen(getMonthExpenseTotal(currentMonth.getFullYear(), currentMonth.getMonth()));
  }
  ...
  formManualSchedule.addEventListener('submit', (e) => {
    ...
    if (isRepeat) {
      ...
      const repeatConflicts = occurrences.flatMap(occDate =>
        findScheduleConflicts(getFormattedDate(occDate), startTime, endTime)
      );
      if (!confirmScheduleConflicts(repeatConflicts, title)) return;
      ...
      const multiDayConflicts = [];
      const conflictCheckDate = new Date(start);
      while (conflictCheckDate <= end) {
        multiDayConflicts.push(...findScheduleConflicts(getFormattedDate(conflictCheckDate), startTime, endTime));
        conflictCheckDate.setDate(conflictCheckDate.getDate() + 1);
      }
      if (!confirmScheduleConflicts(multiDayConflicts, title)) return;
      ...
      const conflicts = findScheduleConflicts(dateStr, startTime, endTime);
      if (!confirmScheduleConflicts(conflicts, title)) return;
...
[
  ...
  function getMonthExpenseTotal(year, month) {
    ...
    return Object.keys(expenses)
      ...
      .reduce((sum, dStr) => sum + expenses[dStr].reduce((s, entry) => s + (entry.amount || 0), 0), 0);
  }

  // 年間表示：12ヶ月分のタイルを描画
  function renderLedgerYearGrid() {
    ledgerYearLabel.textContent = `${ledgerYear}年の支出`;

    const today = new Date();
    ledgerYearGrid.innerHTML = '';

    for (let m = 0; m < 12; m++) {
      const total = getMonthExpenseTotal(ledgerYear, m);
      const isCurrent = ledgerYear === today.getFullYear() && m === today.getMonth();

      const tile = document.createElement('div');
      tile.className = 'ledger-month-tile' + (total === 0 ? ' empty' : '') + (isCurrent ? ' current' : '');
      tile.innerHTML = `
        <span class="ledger-month-tile-label">${m + 1}月</span>
        <span class="ledger-month-tile-amount">${formatYen(total)}</span>
      `;
      tile.addEventListener('click', () => showLedgerMonthDetail(ledgerYear, m));

      ledgerYearGrid.appendChild(tile);
    }
  }

  // 月別詳細：指定した年月の支出を日別グループで描画
  function renderLedgerMonthDetail(year, month) {
    const monthPrefix = `${year}-${String(month + 1).padStart(2, '0')}-`;
  ...
  function showLedgerMonthDetail(year, month) {
    ledgerDetailYear = year;
    ledgerDetailMonth = month;

    ledgerYearView.style.display = 'none';
    ledgerMonthDetail.style.display = '';

    renderLedgerMonthDetail(year, month);
  }

  function backToLedgerYearView() {
    ledgerMonthDetail.style.display = 'none';
    ledgerYearView.style.display = '';

    if (ledgerDetailYear !== null) {
      ledgerYear = ledgerDetailYear;
    }
    renderLedgerYearGrid();
  }

  // 出納帳の現在表示中のビュー（年間 or 月別詳細）とカレンダーの支出サマリーをまとめて更新
  function refreshCashBookViews() {
    calendarMonthExpense.textContent = formatYen(getMonthExpenseTotal(currentMonth.getFullYear(), currentMonth.getMonth()));

    if (ledgerMonthDetail.style.display === 'none') {
      renderLedgerYearGrid();
    } else {
      renderLedgerMonthDetail(ledgerDetailYear, ledgerDetailMonth);
    }
  }

  btnLedgerPrevYear.addEventListener('click', () => {
    ledgerYear -= 1;
    renderLedgerYearGrid();
  });

  btnLedgerNextYear.addEventListener('click', () => {
    ledgerYear += 1;
    renderLedgerYearGrid();
  });

  btnLedgerBackToYear.addEventListener('click', backToLedgerYearView);
  ...
  formManualExpense.addEventListener('submit', (e) => {
    ...
    renderCalendar();
    showLedgerMonthDetail(addedDate.getFullYear(), addedDate.getMonth());
...
[
  ...
  formConfirmReceipt.addEventListener('submit', (e) => {
    ...
    if (lastDateStr) {
      ...
      renderCalendar();
      showLedgerMonthDetail(lastDate.getFullYear(), lastDate.getMonth());
    } else {
      renderCalendar();
    }
